...
const ComplaintTrendChart: React.FC<ChartOneProps> = ({ selectedCategory, data = [] }) => {
    ...
    const trendSeries = React.useMemo(() => {
        ...
        // 백로그 계산 (0부터 시작하여 접수-완료 누적)
        let runningBacklog = 0;
        const backlog = data.map(d => {
            runningBacklog = runningBacklog + (d.received || 0) - (d.completed || 0);
            return runningBacklog;
        });

        // 증감률 계산
        const growth = received.map((val, i) => {
            if (i === 0) return 0;
            const prev = received[i - 1];
            return prev === 0 ? 0 : parseFloat(((val - prev) / prev * 100).toFixed(1));
        });
    ...
    const series = [
        ...(visibility.received ? [{ name: '접수 건수', type: 'area', data: trendSeries.received }] : []),
        ...(visibility.completed ? [{ name: '완료 건수', type: 'area', data: trendSeries.completed }] : []),
        ...(visibility.growth ? [{ name: '증감률 (%)', type: 'line', data: trendSeries.growth }] : []),
        ...(visibility.backlog ? [{ name: '미처리 잔량 (Backlog)', type: 'line', data: trendSeries.backlog }] : []),
    ];
                ...
                <div style={{ textAlign: 'right', backgroundColor: '#FFF7ED', padding: '12px 20px', borderRadius: '12px', border: '1px solid #FFEDD5' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px' }}>
                        <span style={{ fontSize: '24px', fontWeight: '950', color: '#EA580C' }}>
                            {trendSeries.backlog[trendSeries.backlog.length - 1] || 0}
                        </span>
                        {trendSeries.growth.length > 0 && (
                            <span style={{ fontSize: '14px', fontWeight: '900', color: trendSeries.growth[trendSeries.growth.length - 1] >= 0 ? '#EF4444' : '#3B82F6' }}>
                                <span style={{ marginRight: '4px' }}>{trendSeries.growth[trendSeries.growth.length - 1] >= 0 ? '▲' : '▼'}</span>
                                {Math.abs(trendSeries.growth[trendSeries.growth.length - 1])}%
                                <span style={{ fontSize: '12px', color: '#9A3412', fontWeight: '700' }}>(전주 대비)</span>
                            </span>
                        )}
                    </div>
                </div>
