...
const ComplaintCategoryChart: React.FC<ChartTwoProps> = ({ selectedCategory, onSelect, data = [] }) => {
    // 백엔드 데이터(name, value)를 기반으로 차트 데이터 구성
    const chartData = React.useMemo(() => {
        const sorted = [...data].sort((a, b) => b.value - a.value).slice(0, 5);
        return {
            series: sorted.map(d => d.value),
                        ...
                        {data.map((item, i) => (
                            ...
                            >
                                ...
                                <div style={{
                                    ...
                                    fontWeight: '950',
                                    marginRight: '20px',
                                    backgroundColor: (i + 1) <= 5 ? '#A0C4FF' : '#E2E8F0',
                                    color: 'white',
                                    boxShadow: (i + 1) <= 5 ? '0 4px 6px rgba(160, 196, 255, 0.4)' : 'none',
                                    flexShrink: 0
                                }}>
                                    {i + 1}
                                </div>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '20px', flexShrink: 0 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px', minWidth: '100px', justifyContent: 'flex-end' }}>
                                        <span style={{ fontSize: '17px', fontWeight: '900', color: '#334155' }}>{(item.value || 0).toLocaleString()}</span>
                                    ...
                                    <div style={{
                                        ...
                                        fontSize: '13.5px',
                                        fontWeight: '950',
                                        color: (item.change || 0) >= 0 ? '#EF4444' : '#3B82F6',
                                        padding: '5px 10px',
                                        borderRadius: '8px',
                                        backgroundColor: (item.change || 0) >= 0 ? '#FEF2F2' : '#EFF6FF',
                                        ...
                                        <span style={{ fontSize: '11px', display: 'flex', alignItems: 'center' }}>{(item.change || 0) >= 0 ? '▲' : '▼'}</span>
                                        <span style={{ letterSpacing: '-0.5px' }}>{Math.abs(item.change || 0)}%</span>
                                    </div>
                                </div>
                            </div>
                        ))}
