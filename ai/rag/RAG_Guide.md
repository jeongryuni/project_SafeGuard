...
내부 엔진이 최종적으로 내놓는 점수에 대한 3가지 시나리오를 설명합니다.

**시나리오 1: "공원 운동기구가 부러져서 사용이 불가능합니다."**
1.  **질의 힌트 (+5.0)**: `운동기구` 키워드가 '국토교통부'로 힌트 보너스 획득.
2.  **검색 결과 (+1.5)**: `도시공원 및 녹지 등에 관한 법률.pdf` 검색됨.
3.  **최종 결정**: 힌트(5.0) + 검색(1.5) = **6.5점**. 신뢰도 0.85 이상으로 '국토교통부' 확정.

**시나리오 2: "재난 문자가 너무 자주 와서 일에 방해가 돼요."**
1.  **질의 힌트 (+5.0)**: `재난` 키워드가 '행정안전부'로 힌트 보너스 획득.
2.  **MOIS Guard (보류 해제)**: `재난` 단어가 `MOIS_CONTEXT_TERMS`에 포함되어 있어 점수 상한(0.8) 해제.
3.  **검색 결과 (+2.0)**: `재난 및 안전관리 기본법.pdf` 검색됨.
4.  **최종 결정**: 힌트(5.0) + 검색(2.0) = **7.0점**. '행정안전부' 확정.

**시나리오 3: "우리 동네 가로등이 밤에 너무 어두워서 무서워요."**
1.  **질의 힌트 (+5.0)**: `어두워` 키워드가 '국토교통부'로 힌트 보너스 획득.
...
## 13. RAG 시스템 신뢰도(Confidence) 산출 가이드 (Confidence Calculation Guide)

사용자의 질의에 대해 AI 모델이 얼마나 확신을 가지고 결과를 도출했는지를 나타내는 지표인 '신뢰도(Confidence)'의 산출 로직과 검증 예시를 설명합니다.

### 13.1 신뢰도 산출 공식
신뢰도는 검색된 결과들의 점수 합산 중 최종 선택된 기관이 차지하는 비율을 기반으로 계산됩니다.

- **수식**: $Confidence = \frac{Best\_Agency\_Score}{Total\_Agency\_Score}$
- **정규화**: 산출된 값은 소수점 둘째 자리까지 반올림하여 반환합니다(예: 0.85).

### 13.2 신뢰도 보정 및 안전장치
1.  **Confidence Floor (0.45)**: 최종 신뢰도가 0.45 미만인 경우, 시스템은 판단을 유보하고 결과 기관을 **'기타'**로 분류합니다.
2.  **Gap Analysis (0.40)**: 1순위 기관과 2순위 기관의 점수 차이(Gap)가 0.40 미만일 경우에도 **'기타'**로 분류하여 오분류 리스크를 최소화합니다.
3.  **Hard Rule (1.0)**: 불법주정차 등 명확한 키워드 규칙에 의해 분류된 경우, 검색 과정을 생략하고 **1.0(100%)**의 신뢰도를 부여합니다.

---

## 14. 신뢰도 검증 예시 (Verification Examples)

다음은 실제 시스템에서 테스트된 3가지 주요 시나리오와 그에 따른 RAG 분석 결과입니다.

### [질문 1] "어제 식당에서 먹은 밥에서 파리가 나왔어요. 위생 상태가 너무 엉망인데 어디에 신고해야 하나요?"
- **분류 결과**: 식품의약품안전처 (23)
- **신뢰도**: **0.93**
- **판단 근거**: `식당`, `밥`, `위생` 키워드가 식품의약품안전처 힌트와 강력하게 매칭되었으며, `식품위생법` 검색 결과가 높은 가중치를 얻었습니다.
- **JSON 결과**:
  ```json
  {
    "agency_code": 23,
...
### [질문 2] "아파트 앞 도로에 불법 주정차된 차들 때문에 너무 불편합니다. 단속 좀 해주세요."
- **분류 결과**: 경찰청 (18)
- **신뢰도**: **1.0**
- **판단 근거**: `불법` + `주정차` + `단속` 키워드 조합이 **Hard Rule**에 적중하여 즉시 분류되었습니다.
- **JSON 결과**:
  ```json
  {
    "agency_code": 18,
...
### [질문 3] "오늘 기분이 너무 안 좋아서 그런데 누구랑 얘기 좀 하고 싶어요."
- **분류 결과**: 기타 (38)
- **신뢰도**: **0.0**
- **판단 근거**: 행정 업무와 관련된 키워드가 발견되지 않았으며, 검색된 법령 결과의 유사도가 임계값(`MIN_VECTOR_SCORE_THRESHOLD`)을 넘지 못해 분류가 보류되었습니다.
- **JSON 결과**:
  ```json
  {
    "agency_code": 38,
