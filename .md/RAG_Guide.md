...
- **분류 알고리즘 (Logic Flow)**:
  ...
  2.  **강제 규칙 (Hard Rules)**:
      - "불법 주정차" 등 명확한 패턴이 발견되면 복잡한 검색 과정을 생략하고 즉시 '경찰청'으로 분류합니다.
...
2.  **검색 결과 가중치 (Retrieval Score)**
    ...
    *   파일명 기관 매칭: **+0.5** (`KEYWORD_TO_AGENCY`에 '식품' 포함 시 `weight += 0.5`)
    *   힌트 일치 보너스: **+1.0** (파일명 매칭 기관 == 힌트 기관)
    *   기타 가중치: **1.0** (기본 가중치)
    *   **계산**: $1.0(\text{기본}) + 0.84(\text{유사도}) + 0.5(\text{파일명매칭}) + 1.0(\text{힌트일치}) = 3.34$
    
    *   **[추가 예시] "도로가 진짜 지뢰밭입니다"**
        *   질의 힌트: '지뢰밭' 키워드 매칭 -> 국토교통부 (+3.0)
        *   검색 결과: `도로법.pdf` (Vector Score: 0.95)
        *   최종 신뢰도: **0.95+**
        *   효과: 은유적인 표현도 정확하게 국토부로 분류.

3.  **최종 합산**
    *   초기 힌트(3.0) + 검색 가중치(3.34) = **6.34점**
