...
- **주요 기능**:
  ...
  - Prometheus Metric(`process_voice_duration_seconds` 등)을 수집하여 모니터링을 지원합니다.
...
- **분류 알고리즘 (Logic Flow)**:
  ...
  2.  **강제 규칙 (Hard Rules)**: "불법 주정차" 등 명확한 패턴 발견 시 즉시 경찰청으로 분류합니다.
  3.  **검색 (Retrieval)**: `query.py`의 `ask()` 함수를 호출하여 상위 3개의 관련 법령 문서를 가져옵니다.
  4.  **점수 산정 (Scoring)**: 
      - 질의어 힌트 기전(Query Hint)으로 특정 기관에 가점(+5.0)을 부여합니다.
      - 벡터 검색 결과가 임계값(0.63) 미만인 경우 노이즈로 간주하여 제외합니다.
      - 범용 법령(지방자치법 등)은 가중치를 감점(0.35배) 처리합니다.
  5.  **보정 (Guardrails)**: MOIS Guard를 통해 행정안전부 과다 분류를 방지합니다.
  6.  **최종 결정 (Decision)**: 선택된 기관의 비중(Confidence)이 0.45 미만 혹은 1/2위 격차가 0.4 미만이면 '기타'로 분류합니다.
...
- **검색 전략 (Hybrid Search)**:
  - **Vector Search**: `SentenceTransformer`와 Milvus를 사용한 의미 중심 검색. 코사인 거리 값을 점수로 변환(1.0 - distance)하여 정규화합니다.
  - **Keyword Search**: `Kiwipiepy`와 `Rank_BM25`를 사용한 단어 중심 검색. 로컬 역색인 파일(`bm25_index.pkl`)을 사용합니다.
  - **RRF (Reciprocal Rank Fusion)**: 두 검색 결과를 60의 파라미터를 갖는 공식으로 융합합니다.

### 2.4. `test_precision.py` & `verify_classification.py`
**역할**: 분류 성능을 검증하기 위한 테스트 스크립트입니다.
- `test_precision.py`: 상호작용형 터미널 인터페이스를 제공하여 대화형으로 AI 분류 성능을 즉시 테스트할 수 있습니다.
- `verify_classification.py`: 특정 단일 쿼리에 대한 결과 및 근거(Sources)를 CLI로 출력하여 디버깅을 돕습니다.

### 2.5. `ingest.py`
**역할**: PDF 데이터를 로드하여 Vector DB에 적재하고 BM25 색인을 생성합니다.
- PDF에서 텍스트 추출 시 `pdfplumber`를 사용하며, 문맥 보존을 위해 100자 오버랩(Overlap) 설정을 사용합니다.

### 2.6. `milvus_client.py`
**역할**: Milvus 서버(Standalone/Etcd/MinIO)와의 gRPC 통신 및 컬렉션 관리를 담당합니다. `L2` 또는 `COSINE` 거리를 지원하도록 인덱스가 구성됩니다.

### 2.7. `logging_config.py`
**역할**: JSON 구조의 로그 포맷을 설정하여 중앙화된 로그 수집 환경을 지원합니다.

### 2.8. `Dockerfile` & `requirements.txt`
**역할**: Python 3.9 환경 구축 및 의존성 관리.

### 2.9. `rag_data/` (Directory)
**역할**: 원본 PDF 법령 파일(86건)과 로컬 BM25 인덱스 파일이 위치하는 저장소입니다.
- **핵심 파일**: `complaint_examples.md` (AI 학습용 유사 민원 사례집)
